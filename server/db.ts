...
// Synthetic owner used when the database is unavailable so the UI still renders.
export const FALLBACK_OWNER_USER: User = {
  id: 1,
...
// Single-user workspace: every request resolves to the seeded owner user.
export async function getOwnerUser(): Promise<User> {
  const db = await getDb();
  if (!db) return FALLBACK_OWNER_USER;
  const result = await db
    .select()
    .from(users)
    .where(eq(users.openId, OWNER_OPEN_ID))
    .limit(1);
  return result[0] ?? FALLBACK_OWNER_USER;
}
