...
function TaskDrawer({ task, onClose, allTasks }: { task: TaskRecord | null; onClose: () => void; allTasks: TaskRecord[] }) {
  ...
  const writeDescription = trpc.assist.writeDescription.useMutation();
  const suggestLinks = trpc.assist.suggestLinks.useMutation();
  const summarizeChain = trpc.assist.summarizeChain.useMutation();
  const [linkSearch, setLinkSearch] = useState("");
  const [editing, setEditing] = useState(false);
  const [editDraft, setEditDraft] = useState({ title: "", description: "", dueDate: "", assigneeId: "" });
  const [assistError, setAssistError] = useState("");
  const [suggestions, setSuggestions] = useState<Array<{ taskId: number; relationshipType: Relationship; rationale: string }>>([]);
  ...
  const aiBusy = writeDescription.isPending || suggestLinks.isPending || summarizeChain.isPending;
